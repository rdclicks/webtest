 ...
 */
class Fotografie_Featured_Post_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$this->defaults = array(
			...
			'content_limit'   => 200,
			'more_text'       => esc_html__( 'Continue Reading', 'fotografie' ),
		);

		$widget_ops = array(
			'classname'   => 'ct-featured-post ctfeaturedpostpage',
			'description' => esc_html__( 'Displays featured posts with thumbnails', 'fotografie' ),
		);
		...
		parent::__construct(
			...
			esc_html__( 'CT: Recent Posts', 'fotografie' ), // Name
			$widget_ops,
			$control_ops
		);
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'post_number' ); ?>"><?php _e( 'No. of Posts', 'fotografie' ); ?>:</label>
		...
		<p>
        	...
        	<label for="<?php echo $this->get_field_id('disable_image'); ?>"><?php _e( 'Check to Disable Image', 'fotografie' ); ?></label><br />
        </p>

        <p <?php echo ( $instance['disable_image'] )? $style : '';  ?>>
			<label for="<?php echo $this->get_field_id( 'image_alignment' ); ?>"><?php _e( 'Image Alignment', 'fotografie' ); ?>:</label>
			<select id="<?php echo $this->get_field_id( 'image_alignment' ); ?>" name="<?php echo $this->get_field_name( 'image_alignment' ); ?>" class="widefat">
				<?php
					$image_alignment_choices = array(
						'top'    => __( 'Top', 'fotografie' ),
						'left'   => __( 'Left', 'fotografie' ),
						'right'  => __( 'Right', 'fotografie' ),
						'center' => __( 'Center', 'fotografie' ),
					);
		...
		<p <?php echo ( $instance['disable_image'] )? $style : '';  ?>>
			<label for="<?php echo $this->get_field_id( 'image_size' ); ?>"><?php _e( 'Image Size', 'fotografie' ); ?>:</label>
			<select id="<?php echo $this->get_field_id( 'image_size' ); ?>" class="fotografie-image-size-selector" name="<?php echo $this->get_field_name( 'image_size' ); ?>">
				<?php
				$sizes = fotografie_get_additional_image_sizes();
        ...
        <p>
        	...
        	<label for="<?php echo $this->get_field_id('disable_title'); ?>"><?php _e( 'Check to Disable Title', 'fotografie' ); ?></label><br />
        </p>

        <p>
			<span class="description"><?php _e( 'Post Meta Info', 'fotografie' ); ?></span><br/>
        	...
        	<label for="<?php echo $this->get_field_id('hide_category'); ?>"><?php _e( 'Check to Hide Category', 'fotografie' ); ?></label><br />
        	...
        	<label for="<?php echo $this->get_field_id('hide_tags'); ?>"><?php _e( 'Check to Hide Tags', 'fotografie' ); ?></label><br />
        	...
        	<label for="<?php echo $this->get_field_id('hide_posted_on'); ?>"><?php _e( 'Check to Hide Posted On Date', 'fotografie' ); ?></label><br />
        	...
        	<label for="<?php echo $this->get_field_id('hide_author'); ?>"><?php _e( 'Check Hide to Author', 'fotografie' ); ?></label><br />
		</p>

        <p>
			<label for="<?php echo $this->get_field_id( 'content_type' ); ?>"><?php _e( 'Content Type', 'fotografie' ); ?>:</label>
			<select class="ct_feat_post_content_type" id="<?php echo $this->get_field_id( 'content_type' ); ?>" name="<?php echo $this->get_field_name( 'content_type' ); ?>">
				<option value="content" <?php selected( 'content', $instance['content_type'] ); ?>><?php _e( 'Show Content', 'fotografie' ); ?></option>
				<option value="excerpt" <?php selected( 'excerpt', $instance['content_type'] ); ?>><?php _e( 'Show Excerpt', 'fotografie' ); ?></option>
				<option value="content-limit" <?php selected( 'content-limit', $instance['content_type'] ); ?>><?php _e( 'Show Content Limit', 'fotografie' ); ?></option>
				<option value="" <?php selected( '', $instance['content_type'] ); ?>><?php _e( 'No Content', 'fotografie' ); ?></option>
			</select>
			<br />
			<label <?php echo ( 'content-limit' != $instance['content_type'] )? $style : '';  ?>for="<?php echo $this->get_field_id( 'content_limit' ); ?>"><?php _e( 'Limit content to', 'fotografie' ); ?>
				...
				<?php _e( 'characters', 'fotografie' ); ?>
			</label>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'more_text' ); ?>"><?php _e( 'More Text (if applicable)', 'fotografie' ); ?>:</label>
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['disable_image']   = fotografie_sanitize_checkbox( $new_instance['disable_image'] );
		...
		$instance['disable_title']   = fotografie_sanitize_checkbox( $new_instance['disable_title'] );
		$instance['hide_category']   = fotografie_sanitize_checkbox( $new_instance['hide_category'] );
		$instance['hide_tags']       = fotografie_sanitize_checkbox( $new_instance['hide_tags'] );
		$instance['hide_posted_on']  = fotografie_sanitize_checkbox( $new_instance['hide_posted_on'] );
		$instance['hide_author']     = fotografie_sanitize_checkbox( $new_instance['hide_author'] );
			...
			$output .= '<div class="article-wrap">';
				while ( $loop->have_posts() ) : $loop->the_post();
					...
					<article class="post-'. esc_attr( get_the_ID() ) . ' post type-post hentry">';
						...
						$output .= fotografie_get_highlight_meta(
									$instance['hide_category'],
									$instance['hide_tags'],
									$instance['hide_posted_on'],
									$instance['hide_author']
						...
						elseif ( 'content-limit' == $instance['content_type'] ) {
							$output .= '<div class="entry-summary">';
							$output .= fotografie_get_the_content_limit( (int) $instance['content_limit'], esc_html( $instance['more_text'] ) );
							$output .= '</div><!-- .entry-summary -->';
						}
 ...
 */
function fotografie_register_featured_post_widget() {
    register_widget( 'Fotografie_Featured_Post_Widget' );
}
add_action( 'widgets_init', 'fotografie_register_featured_post_widget' );
